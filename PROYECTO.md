...
16. **Pulido** — partido en sub-fases temáticas para mantener scope acotado:
    ...
    - **16e.** Pulido del editor. **Partido en cuatro sub-pasos independientes**, cada uno cierra por sí solo con su entrada de roadmap y, si corresponde, actualización de `docs/deferred.md`.
      ...
      - **16e-ii.** Re-mapping de offsets de `range` ante ediciones del bloque. Hoy (desde 13g-i) `Comment.range?: { from, to }` se congela al valor de creación: el renderer clampa a fin de bloque pero no aplica `tr.mapping` cuando el texto cambia. Requiere un plugin TipTap que propague cada `tr.mapping` a los anchors persistidos en memoria y los flushee a disco vía `CommentsService` en el próximo autosave, más orphan-flag cuando el range colapsa (`to <= from`). Resuelve la entrada "Re-mapping de offsets de `range`..." de `docs/deferred.md`.
