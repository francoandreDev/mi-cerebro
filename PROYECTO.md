...
16. **Pulido** — partido en sub-fases temáticas para mantener scope acotado:
    ...
    - **16b.** Pulido visual del árbol: scroll automático al match activo, lista de coincidencias desplegada navegable con teclado, drag & drop para reordenar nodos. **Partido en seis sub-pasos independientes**, cada uno cierra por sí solo con su entrada de roadmap y, si corresponde, actualización de `docs/deferred.md`.
      ...
      - **16b-v. D&D intra-folder en el árbol.** HTML5 drag API en `TreeNodeComponent` con `draggable=true` sobre rows de entidad (no folder por ahora). Drop targets: gap entre rows del mismo padre, con **línea de inserción** visible (border-top/bottom + `--mc-accent-primary`). Suelta → `setPosition(id, between(prev?.position, next?.position))`. Restricciones: drag sólo dentro del mismo kind, mismo folder padre, mismo nivel; cualquier drop fuera de esos límites cancela visualmente y no persiste. A11y: `aria-grabbed`/`aria-dropeffect` (o equivalente moderno con `aria-live` para confirmación), navegación alternativa por teclado queda fuera de scope de este sub-paso (cae en 16b-vi si surge dolor).
      - **16b-vi. D&D cross-folder + reorden de folders.** Cierra 16b. Drop sobre un folder row → mueve la entidad ahí y la posiciona al final del nuevo padre (`moveToFolder` + `setPosition(id, between(lastInTarget, null))`). Drop entre rows de otro folder → mueve + posición exacta entre los vecinos del destino. Folders también ganan `draggable=true` y `setPosition` (mismo helper), pueden reordenarse dentro del mismo padre y moverse a otros folders del mismo kind. Restricción: prohibido drop de un folder dentro de un descendiente propio (validado en `dragover` con cancel visual). Resuelve la entrada "Drag & drop / reordenamiento" de `docs/deferred.md`.
