...
16. **Pulido** — partido en sub-fases temáticas para mantener scope acotado:
    ...
    - **16e.** Pulido del editor. **Partido en cuatro sub-pasos independientes**, cada uno cierra por sí solo con su entrada de roadmap y, si corresponde, actualización de `docs/deferred.md`.
      ...
      - **16e-iii.** Anchor `range` multi-bloque: soporta selecciones de comentario que cruzan dos o más párrafos. Requiere modelo de anchor distinto (lista de `{ blockId, from, to }` o par `{ startBlockId, startOffset, endBlockId, endOffset }`), su propio orphan handling (bloque del medio borrado) y renderer que dibuje la nube en el último bloque del span. Resuelve la entrada "Anchor `range` multi-bloque..." de `docs/deferred.md`.
      - **16e-iv.** Widget render para diff-marks "insertion-only": mini-renderer JSON→DOM consistente con el theme del editor para pintar inline los diff-marks que insertan bloques enteros sin anchor en `main` (hoy sólo listados en el popover de pendientes). Resuelve la entrada "Widget render para diff-marks de tipo insertion-only" de `docs/deferred.md`.
