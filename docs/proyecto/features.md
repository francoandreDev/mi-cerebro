...
Sistema combinado de autocommits + **variantes** (ramas renombradas en UI) + panel de historial navegable, sobre **isomorphic-git** con adapter propio a File System Access API para el workdir del usuario y OPFS para `.git/` en sí (ver "Costo de operaciones git" más abajo). El export ZIP (paso 14) coexiste como snapshot manual rápido fuera del flujo git.
...
**Proxy CORS para push/fetch a GitHub.** El navegador no puede hablarle directo a los endpoints smart-HTTP de GitHub (sin headers CORS del lado de GitHub), así que isomorphic-git rutea push/fetch a través de un proxy que los agrega. Por defecto se usa el proxy público `cors.isomorphic-git.org` (mismo que usa la librería out-of-the-box) — sigue siendo la opción sin fricción, no está señalado como inseguro. Agregado 2026-08-06: `RemoteConfig.corsProxyUrl` (opcional, `remote.types.ts`) permite apuntar a un proxy propio en su lugar; `remote-bulk.ts` cae al público cuando el campo está ausente (`corsProxyFor()`). UI en `/settings` → "Versionado remoto" → "Proxy CORS propio (opcional)", validado con `isValidProxyUrl` (cualquier origin `https://`, no scopeado a un host como `isValidRemoteUrl`). Implementación de referencia lista para deployar en `infra/cors-proxy-worker/` (Cloudflare Worker, port del proxy oficial de isomorphic-git pero restringido a `github.com` únicamente en vez de proxy abierto a cualquier host — ver comentario `ALLOWED_HOST` en `worker.js`) con README paso a paso (creación de cuenta Cloudflare incluida, plan gratis cubre el uso de un solo usuario sin costo). El deploy en sí es una acción del usuario, no automatizada desde la app.

### Costo de operaciones git — `.git/` en OPFS, workdir en FS Access

Cada operación de isomorphic-git sobre el adapter FS Access tiene un piso de ~100-200 ms por syscall del browser. Movido 2026-08-08: `.git/` (objects/refs/index) vive en el **Origin Private File System** del browser en vez de la carpeta real del usuario — el workdir (notas, tareas, todo lo que el usuario ve en su carpeta) sigue en FS Access, sin cambios. isomorphic-git acepta `dir` (workdir) y `gitdir` como paths separados dentro de un mismo `fs`; como no soporta dos filesystems distintos directamente, `GitFsAdapter` (`core/versioning/git-fs.adapter.ts`) toma un `gitDirRoot` opcional en el constructor y rutea internamente cualquier path bajo `/.git` a esa raíz en vez de la del workdir — ningún caller (`VersioningService`, `SwitchVariantService`, etc.) pasa un `gitdir` explícito, todos siguen con `dir: '/'` como antes; el default de isomorphic-git (`gitdir = dir + '/.git'`) es exactamente el prefijo que la routing intercepta.

`BrowserNativeFs` resultó reutilizable tal cual para el lado OPFS — sólo llama a la interfaz estándar `FileSystemDirectoryHandle`/`FileSystemFileHandle` (`getDirectoryHandle`, `getFileHandle`, `entries()`, `createWritable()`, `.move()`), que `navigator.storage.getDirectory()` implementa igual. `OpfsGitRootService` (`core/versioning/opfs-git-root.ts`) resuelve y cachea ese directorio; gateado a `PlatformService.current === 'browser'` — Tauri/Capacitor ya tienen FS nativo rápido, mover `.git` ahí no gana nada.

**Migración one-time** (`VersioningService.migrateGitDirToOpfsIfNeeded()`, corre dentro de `ensureRepo()` antes de cualquier otra cosa): si el usuario ya tenía `.git` en la carpeta real de una sesión anterior a este cambio, se copia entero a OPFS, se verifica leyendo `HEAD` a través del adapter combinado, y sólo entonces se borra el original — nunca se toca el `.git` real antes de confirmar que la copia es legible (mismo criterio que `writeAtomicViaTmp`). Corre bajo `FsLockService` para no cruzarse con autocommit.

**Implicaciones:** el export ZIP (paso 14, `ExportZipService`) camina también el `.git` de OPFS cuando "incluir todas las variantes" está activo — antes sólo caminaba la carpeta real. Si el usuario limpia datos del sitio en el navegador, pierde el historial git (las notas en sí no, viven en FS Access); recuperable desde GitHub si tenía push configurado (13e). Riesgo aceptado sin mitigación adicional: es el mismo trade-off que ya existía para IndexedDB (temas custom, índice de búsqueda).

Las operaciones de **autocommit** corren en background y el costo (ahora bajo, ver benchmark) es invisible igual. Las operaciones **disparadas por el usuario** (switch de variante, merge entre variantes, accept de un diff-mark del borrador, crear/borrar variante) siguen mostrando la misma **pantalla de carga con mensaje contextual** — se mantiene el patrón aunque el piso bajó, no vale la pena quitarla por una operación que sigue siendo async.

`DevPerfService.runOpfsComparison(n)` (dev-only) corre el mismo caso de commit+log+readBlob dos veces — una vez con `.git` en FS Access, una vez con `.git` en OPFS — y reporta el speedup real medido en la máquina del usuario, en vez de asumir el 10-100× estimado antes de implementar.
...
- **Nota libre + "ronroneo" al hover sostenido** (cierra el ítem "Detalles bonitos: ronroneo" de `deferred/reminders-goals.md`): `Reminder.note: string` (schema v6, migración v5→v6 default `''`), editable como textarea en el panel de detalle junto al título. Mantener el cursor ~650ms sobre una paloma (nicho o repisa) muestra `mc-paloma-note-preview`, una burbuja flotante con la nota — si está vacía, cae al título en vez de mostrar nada vacío (la UI no inventa contenido que no existe). Sin patrón de dwell-hover previo en el repo: temporizador armado en `mouseenter`/cancelado en `mouseleave` dentro de `RemindersContainer`, sin nuevo servicio.
