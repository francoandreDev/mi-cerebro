...
- **`features/<entity>/`** estructura espejada:
  ...
  ```
- **`core/`** por dominio: `fs/`, `idb/`, `index/`, `theme/`, `continuity/`, `reminders/`, `versioning/`, `autosave/`, `errors/`, `migrations/`, `tags/`, `relations/` (conexiones y backlinks entre entidades, ver §10bis en `features.md`), `i18n/`, `dnd/` (auto-scroll de borde durante drag-and-drop nativo, transversal a toda la app — ver `DragAutoScrollService`), `tutorials/`.
...
**Varios flujos por página, seleccionables.** Una página puede registrar más de un `TutorialDefinition` — `id` sigue siendo la clave única (arranque + seen-tracking), pero `pageId` (nuevo campo) agrupa definiciones para el botón ✨: si `TutorialService.tutorialsForPage(pageId)` devuelve una sola, "Guía de la página" la arranca directo (comportamiento de siempre); si devuelve varias, abre un popover (`layout/components/tutorial-picker-menu.component.ts`) listando el `labelKey` de cada una. Solo la definición pensada como flujo principal de la página debe registrarse con `autoStartIfUnseen: true` — el resto son manuales, descubribles únicamente desde el picker. Cambiar de tab dentro de una página (Settings: 9, Music: 2, ambas con estado local sin URL) no necesita capacidad nueva de engine: un step con `action: { event: 'click', selector: '[data-tutorial="..."]' }` sobre el botón de la tab ya resuelve "practicá cambiar de tab" (mismo mecanismo que cualquier otro gesto real), y el step siguiente —cuyo anchor solo existe dentro de esa tab— se marca `skipIfMissing: true` para el caso de que el usuario avance con "Siguiente" sin practicar el click. Ver roadmap ítem 26, Fase 8, 8.18.
