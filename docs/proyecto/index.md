...
| **Escritos largos**     | Libros, artículos, apuntes de idiomas, conocimiento de un tema, planes. Editor cómodo. El lector/editor de capítulos de libros (`/books/:id/leer`) suma "modo dibujo": trazo a mano alzada por página física del spread (mismo sistema `shared/chalk/` que "modo tiza" de listas — ver §11 en `features.md`), persistido en `Chapter.pageDrawings`.                                             |
