...
- **Diseño decidido (2026-07-15, sin implementar)**: reminder real, no un banner nuevo aparte — homogéneo con el resto del palomar (todas las palomas hoy son `Reminder` reales). Nuevo `sourceKind: 'goal-dormant'`, creado **una sola vez** al detectar el flanco `false → true` de `isGoalDormant` (comparar contra estado anterior en un `effect()`, mismo espíritu que `GoalRemindersSyncService.computeDiff` pero disparando en el cruce, no mientras se mantenga dormant). Reminder puntual (`dueAt: now`, no recurrente) — la dormancia no tiene fecha objetivo natural, así que no encaja en el modelo de lead-up de `RemindersCadenceService`. Se borra automáticamente si el usuario retoma progreso antes de atenderlo, mismo ciclo de vida que ya maneja `GoalRemindersSyncService` para deadlines. Gateado por un nuevo flag propio, ej. `goal.reminder.notifyOnDormant: boolean`, **off por default** — comportamiento nuevo no pedido explícitamente en metas existentes, mejor que lo prenda quien lo quiera (evita el "banner permanente" que §13 prohíbe).
- **Target**: sin asignar.
