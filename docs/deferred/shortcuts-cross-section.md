...
- **Estado**: cerrado. Re-audit completo de todos los `(keydown...)` en templates y `@HostListener('keydown...)` en TS del proyecto:
  ...
  - **Deuda menor no bloqueante, no parte de este ítem**: `history.container.ts` (`[`/`]`) y `shared/editor/editor.component.ts` (Alt+C/Alt+P) usan `@HostListener('document:keydown')` en vez de `ShortcutsService` pese a no ser el patrón "un solo elemento focuseable" que permite la regla como ad-hoc — ambos sí llaman `preventDefault()`, así que no hay bug de default del navegador, sólo una desviación arquitectónica menor. No se tocó en este cierre (fuera del alcance de "defaults del navegador").
    - **Actualización 2026-07-20**: `history.container.ts` migrado a `ShortcutsService` (`combo: '['`/`']'`, `scope: 'editable-safe'`, junto a los `+`/`-`/`Escape` de zoom ya registrados ahí) — encajaba 1:1 porque el guard manual (`INPUT`/`TEXTAREA`/`SELECT`/`isContentEditable`) es exactamente lo que hace `scope: 'editable-safe'` (el único caso no cubierto, `SELECT`, no aplica: la plantilla de `/history` no tiene ningún `<select>`). `shared/editor/editor.component.ts` (Alt+C/Alt+P) **sigue sin migrar, a propósito**: su guard no es "no estoy en un campo editable" sino "el evento ocurrió dentro de esta instancia de editor" (`this.host().nativeElement.contains(target)`), y el contenido del editor es él mismo `contenteditable` — con `scope: 'editable-safe'` el atajo nunca dispararía (`ShortcutsService` trata todo `isContentEditable` como campo editable y lo salta), y con `scope: 'global'` dispararía desde cualquier input/editor de la página, no sólo el enfocado. `ShortcutsService` no tiene hoy un scope "dentro de esta instancia de componente" — agregarlo sería una feature nueva, no una migración mecánica. Queda documentado, no bloqueante.
- **Target**: cerrado.
