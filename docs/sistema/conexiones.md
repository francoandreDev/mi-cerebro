...
- **Puntos de entrada:** botón fijo "Vincular a…" en la barra del editor (`editor-toolbar.component.ts`), siempre visible cuando el editor es editable — no depende de selección ni de la vista `combined`. También disponible como entrada de la bubble menu cuando hay selección de texto en vista `combined` (dos caminos al mismo `EditorComponent.triggerLink()`). Con selección, el chip envuelve el texto elegido (ese texto pasa a ser el `contextSnippet`); sin selección, inserta el chip en el cursor sin snippet. El mismo botón está replicado en `chapter-editor-pane.component.ts` para capítulos de libro (que esconden el toolbar interno de `mc-editor` por su paginación).
  - **Trigger `@` inline:** `core/tiptap/entity-mention/entity-mention.ext.ts` registra un `InputRule` (`(?:^|\s)@$`, no dispara a mitad de palabra — un email `user@host` no abre el picker) que, al tipear `@` al inicio de una palabra, llama `EditorComponent.onEntityMentionTrigger()` → mismo `openLinkPickerFor()` que usa `triggerLink()`. Deliberadamente **no** se construyó sobre `@tiptap/suggestion` (la base habitual de extensiones "mention"): ese paquete trackea una query que crece mientras se sigue tipeando y arma una lista flotante inline — acá el picker es el mismo diálogo centrado del botón, que roba foco apenas se abre (`mcAutofocus` en su input de búsqueda), así que ningún carácter posterior al `@` vuelve a llegar al editor. El handler deja el `@` como texto plano en el doc; el `range` que dispara sólo cubre ese carácter, así que elegir una entidad lo reemplaza (`insertContentAt`); cerrar sin elegir deja el `@` como texto normal, igual que cualquier tecla no consumida.
