...
- **Pantalla `/reminders`:** quick-add con parseo de fecha en lenguaje natural (`parseQuickAdd`/`parseDueHint`, `features/reminders/utils/parse-due.ts`) — hoy/mañana/pasado mañana/pasado, día de la semana (`viernes`, con o sin el sufijo redundante `que viene`), `fin de semana` (próximo sábado, o el mismo día si ya es sábado), `proximo mes` (mismo día del mes siguiente), fecha absoluta `dd/mm` o `dd/mm/aaaa` (rueda al año siguiente si la fecha sin año ya pasó), `en Nh/Nm/Nd`, y hora final en 24h o am/pm — búsqueda de texto y filtro por rango de fechas, panel de detalle al seleccionar una paloma (título si no viene de fuente, `dueAt`, recurrencia + intervalo, pausa), registro plegable de los `done`. Menú de acciones por paloma (posponer con presets — horas, próximo lunes, próximo fin de semana —, duplicar, borrar). Borrar uno normal hace soft-delete con toast de deshacer (6 s). También tiene **modo lista** con atajos J/K/Space/E/Del — ver [`conexiones.md`](./conexiones.md), que documenta ese primitivo compartido (`createListCursor`) porque reminders fue el primero en adoptarlo.
